...
class PlaywrightExecutor:
        ...
        async with async_playwright() as p:
            try:
                ...
                for i, action in enumerate(actions):
                    logger.info(f"액션 {i+1}/{len(actions)} 실행: {action.action_type}")
    ...
    async def _execute_single_action(self, action: NavigationAction) -> NavigationResult:
        ...
        try:
            ...
            logger.info(f"액션 성공: {action.action_type} (소요 시간: {result.execution_time:.2f}초)")
            
            return result
            
        except Exception as e:
            logger.error(f"액션 실행 실패: {action.action_type}, 오류: {e}")
    ...
    async def _action_click(self, action: NavigationAction):
        ...
        await asyncio.sleep(1)  # 페이지 변화 대기
